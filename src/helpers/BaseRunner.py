...
class BaseRunner(object):
    ...
    def parse_runner_args(parser):
        ...
        parser.add_argument(
            "--metric", type=str, default="NDCG,HR", help="metrics: NDCG, HR"
        )
        parser.add_argument(
            "--main_metric",
            type=str,
    ...
    @staticmethod
    def evaluate_method(
        predictions: np.ndarray, topk: list, metrics: list
    ) -> Dict[str, float]:
        """
    ...
    def __init__(self, args):
        ...
        self.main_metric = (
            "{}@{}".format(self.metrics[0], self.topk[0])
            if not len(args.main_metric)
            else args.main_metric
        )  # early stop based on main_metric
        self.main_topk = (
            int(self.main_metric.split("@")[1]) if "@" in self.main_metric else 0
        )
    ...
    def train(self, data_dict: Dict[str, BaseModel.Dataset]):
        ...
        logging.info(
            os.linesep
            + "Best Iter(dev)={:>5}\t dev=({}) [{:<.1f} s] ".format(
                best_epoch + 1,
                utils.format_metric(dev_results[best_epoch]),
    ...
    def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
        ...
        for batch in tqdm(
            ...
            batch["item_id"] = item_ids[
                torch.arange(item_ids.shape[0]).unsqueeze(-1), indices
            ]
            ...
            if len(prediction.shape) == 2:  # only for ranking tasks
                restored_prediction = torch.zeros(*prediction.shape).to(
                    prediction.device
                )
    ...
    def predict(
        self, dataset: BaseModel.Dataset, save_prediction: bool = False
    ) -> np.ndarray:
        """
        ...
        if dataset.model.test_all:
            ...
            for i, u in enumerate(dataset.data["user_id"]):
                clicked_items = list(
                    dataset.corpus.train_clicked_set[u]
                    | dataset.corpus.residual_clicked_set[u]
                )
