...
from models.general.AblationStagewiseGCN import AblationStagewiseBase
from utils import utils


class AblationStagewiseRunner(BaseRunner):
    def train(self, data_dict: Dict[str, BaseModel.Dataset]) -> None:
        model: AblationStagewiseBase = data_dict["train"].model
        main_metric_results, dev_results = list(), list()
        self._check_time(start=True)

        try:
            stage = model.enabled_stage
